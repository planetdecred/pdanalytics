...
func (pg *PgDb) mempoolDataTableExits() bool {
	exists, _ := dbhelper.TableExists(pg.db, "mempool")
	return exists
}

func (pg *PgDb) mempoolBinDataTableExits() bool {
	exists, _ := dbhelper.TableExists(pg.db, "mempool_bin")
	return exists
}
...
func (pg *PgDb) DropTables() error {
	...
	// mempool
	if err := dbhelper.DropTable(pg.db, "mempool"); err != nil {
		return err
	}

	// mempool_bin
	if err := dbhelper.DropTable(pg.db, "mempool_bin"); err != nil {
		return err
	}
